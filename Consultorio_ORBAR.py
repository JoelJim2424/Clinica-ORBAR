...
def generar_receta_pdf_varios(nombre_paciente, edad, lista_medicamentos, indicaciones):
    ...
    pdf.cell(0, 8, f'Paciente: {nombre_paciente}', 0, 1)
    pdf.cell(0, 8, f'Edad: {edad} anos', 0, 1)
    ...
    # Este for es para recorrer la lista y pintar todos los meds
    for i, item in enumerate(lista_medicamentos, 1):
        pdf.multi_cell(0, 8, f"{i}. {item['medicamento']} - Dosis: {item['dosis']}")
    
    pdf.ln(5)
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, 'Indicaciones:', 0, 1)
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell(0, 8, indicaciones)
    
    pdf.ln(15)
    pdf.cell(0, 8, '_________________________', 0, 1, 'C')
    pdf.cell(0, 8, 'Firma del Medico', 0, 1, 'C')
