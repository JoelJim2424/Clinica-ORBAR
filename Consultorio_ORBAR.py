...
def a_jon(dato):
    if isinstance(dato, dict):
        return {k: a_jon(v) for k, v in dato.items()}
    if isinstance(dato, (list, tuple)):
        return {k: a_jon(v) for v in dato}
...
def insertar_cita(id_paciente, fecha, hora, dentista, motivo):
    try:
        ...
        resultado = supabase.table("citas").insert(a_jon(datos)).execute()
...
def insertar_pago(id_paciente, id_cita, concepto, monto):
    try:
        ...
        resultado = supabase.table('pagos').insert(a_jon(datos)).execute()
...
def insertar_receta(id_paciente, id_cita, medicamento, dosis, indicaciones):
    try:
        datos = {
            ...
            "fecha_receta": date.today()
        }
        resultado = supabase.table('recetas').insert(a_jon(datos)).execute()
